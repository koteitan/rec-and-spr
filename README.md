...
```sh
mkdir -p transcript
./whisperx-local out.wav --output_dir transcript --output_format all
```
...
```sh
WHISPERX_MODEL=small.en WHISPERX_BATCH_SIZE=2 ./whisperx-local out.wav
```
...
`loopbackrec`が生成されます。デバイス一覧を取得できない場合は、`pactl info`でPipeWire/PulseAudioへの接続を確認してください。
...
```sh
WHISPERX_DEVICE=cpu WHISPERX_COMPUTE_TYPE=int8 ./whisperx-local out.wav
```
